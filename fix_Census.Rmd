...
ifelse(!dir.exists(paste0(file.path('data'))), dir.create(file.path(paste0('data'))), FALSE)
ifelse(!dir.exists(paste0(file.path('data/water_polygons'))), 
       dir.create(file.path(paste0('data/water_polygons'))), FALSE)
```
...
ifelse(!dir.exists(paste0(file.path(out_dir))),
       dir.create(file.path(paste0(out_dir))), FALSE)

ifelse(!dir.exists(paste0(file.path(out_dir), '_no_water')),
       dir.create(file.path(paste0(file.path(out_dir), '_no_water'))), FALSE)
...
tic(); cbg_ply <- get_acs(
  ...
  filter(ALAND > 0) %>%        # drop reas without land
...
tic(); for(i in state_code_abb$state){
  ...
  ifelse(!dir.exists(paste0(file.path('data/water_polygons/', i))),
         dir.create(file.path(paste0('data/water_polygons/', i))), FALSE)
...
tic(); for(i in state_code_abb$state){ # Rhode Island, used for testing 46 = Vermont
# tic(); for(i in state_code_abb$state[c(40, 46)]){ # Rhode Island, used for testing 46 = Vermont
  ...
  ifelse(!dir.exists(paste0(file.path(out_dir), '_no_water/', i)),
       dir.create(file.path(paste0(file.path(out_dir), '_no_water/', i))), FALSE)
  ...
  cbg_ply_state_i_no_water %>%
    bind_rows() %>%
    st_collection_extract(., "POLYGON") %>%                 # cuts out linestrings caused by slivers
  ...
  }; toc() # all 52 places takes ~4 - 4.5 hours
...
tic(); (
  tibble(filename = files) %>% #slice(1:20) |> 
    ...
    # drop slivers, if any
    filter(!is.na(area_multi_prop)) -> cbg_ply_no_water_singlepart #cbg_ply_muliparts
